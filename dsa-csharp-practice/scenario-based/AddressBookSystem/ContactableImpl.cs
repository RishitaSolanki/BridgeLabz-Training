...
namespace ScenarioBased.AddressBookSystem
{
        ...
        {
            int maxLen = 1000;
            public void AddContact(AddressBook addressBook, EContact contact)
            {
            ...
            public void EditContact(AddressBook addressBook, EContact contact, string firstName, string lastName, string address, string city, string state, string zip, string phoneNumber, string eMail)
            {
                for (int i = 0; i < maxLen; i++)
                {
            ...
            public EContact ShowContact(AddressBook addressBook, String searchName)
            {
                for (int i = 0; i < maxLen; i++)
                {
                    if (addressBook.contacts[i] != null)
            ...
            public void DeleteContact(AddressBook addressBook, EContact contact)
            {
                for (int i = 0; i < maxLen; i++)
                {
                    if (addressBook.contacts[i] == contact)
            ...
            public void SortContacts(AddressBook addressBook)
            {
                for (int i = 0; i < maxLen - 1; i++)
                {
                    ...
                    {
                        EContact c1 = addressBook.contacts[j];
                        EContact c2 = addressBook.contacts[j + 1];
